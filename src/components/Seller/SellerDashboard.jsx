  ...
  useEffect(() => {
    const fetchUserPlots = async () => {
      try {
        ...
        setUserPlots(plots.filter((plot) => plot.seller_id === user.id));
      } catch (error) {
...
<<<<<<< HEAD
    ...
    try {
      const newPlot = await addNewPlot({
        ...
        price: parseLakhsToRupees(Number(formData.price)),
        price_per_sqft: Math.round(parseLakhsToRupees(Number(formData.price)) / Number(formData.area_sqft)),
        images: imagePreviews,
      });
      setPendingPlotId(newPlot._id || newPlot.id);
      setShowForm(false);
      setShowPayment(true);
    } catch (error) {
...
<<<<<<< HEAD
  const handleDelete = async (plotId) => {
    ...
    try {
      ...
      alert('Listing not found or failed to delete');
    }
  };
  return (<div className="min-h-screen bg-slate-50">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => setShowForm(true)} className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-3 rounded-xl font-medium hover:from-emerald-700 hover:to-teal-700 transition-all shadow-lg hover:shadow-xl flex items-center space-x-2">
          <Plus className="w-5 h-5" />
          <span>List New Property</span>
        </button>
      </div>

      {user?.kyc_status !== 'verified' && (<div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-8">
      ...
      {showForm && (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 z-10">
            ...
            <div className="flex items-center space-x-2">
              {[1, 2, 3].map((step) => (<div key={step} className="flex items-center flex-1">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full font-semibold text-sm ${currentStep >= step
                  ? 'bg-emerald-600 text-white'
                  : 'bg-slate-200 text-slate-600'}`}>
                  {step}
                </div>
                <div className={`flex-1 h-1 mx-2 rounded ${currentStep > step ? 'bg-emerald-600' : 'bg-slate-200'} ${step === 3 ? 'hidden' : ''}`}></div>
              </div>))}
            </div>
          ...
          <form onSubmit={handleSubmit} className="p-6">
            {currentStep === 1 && (<div className="space-y-6">
              ...
              <div>
                ...
                <input type="text" value={formData.owner_name} onChange={(e) => setFormData({ ...formData, owner_name: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="Enter your full name exactly as on Aadhaar" required />
              </div>
              ...
              <div>
                ...
                <input type="text" value={formData.owner_aadhaar} onChange={(e) => {
                  const value = e.target.value.replace(/\D/g, '').slice(0, 12);
                  setFormData({ ...formData, owner_aadhaar: value });
                }} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="Enter 12-digit Aadhaar number" maxLength={12} required />
              ...
              <div>
                ...
                <input type="text" value={formData.property_owner_name} onChange={(e) => setFormData({ ...formData, property_owner_name: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="Enter name as shown on property documents" required />
              </div>

              {formData.owner_name && formData.property_owner_name && (<div className={`p-4 rounded-lg border-2 ${formData.owner_name.toLowerCase().trim() === formData.property_owner_name.toLowerCase().trim()
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={() => setShowForm(false)} className="px-6 py-2 border-2 border-slate-300 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors">
                  Cancel
                </button>
                <button type="button" onClick={handleNext} className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition-colors">
                  Next Step
                </button>
              </div>
            </div>)}
            ...
            {currentStep === 2 && (<div className="space-y-6">
              <div>
                ...
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="e.g., Premium Residential Plot in Whitefield" required />
              </div>
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={4} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none resize-none" placeholder="Describe the property features, location benefits, nearby amenities..." />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.location_address} onChange={(e) => setFormData({ ...formData, location_address: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="Street address" required />
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={formData.city} onChange={(e) => setFormData({ ...formData, city: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="City name" required />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.state} onChange={(e) => setFormData({ ...formData, state: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="State name" required />
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" value={formData.length_ft} onChange={(e) => {
                        const length = e.target.value;
                        const width = formData.width_ft;
                        const area = length && width ? String(Number(length) * Number(width)) : '';
                        setFormData({ ...formData, length_ft: length, area_sqft: area });
                      }} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="60" required />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-2">
                        Width (ft) <span className="text-red-500">*</span>
                      </label>
                      <input type="number" value={formData.width_ft} onChange={(e) => {
                        const width = e.target.value;
                        const length = formData.length_ft;
                        const area = length && width ? String(Number(length) * Number(width)) : '';
                        setFormData({ ...formData, width_ft: width, area_sqft: area });
                      }} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="40" required />
                    </div>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg">
                    ...
                    <div className="text-lg font-semibold text-slate-900">
                      {formData.area_sqft ? `${Number(formData.area_sqft).toLocaleString('en-IN')} sq ft` : '---'}
                    </div>
                    <p className="text-xs text-slate-500 mt-1">Automatically calculated from length × width</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <input type="number" step="0.01" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none" placeholder="96" required />
                {formData.price && formData.area_sqft && (<p className="text-sm text-slate-600 mt-2">
              ...
              <div className="flex justify-between pt-4">
                <button type="button" onClick={() => setCurrentStep(1)} className="px-6 py-2 border-2 border-slate-300 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors">
                  Back
                </button>
                <button type="button" onClick={handleNext} className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition-colors">
                  Next Step
            ...
            {currentStep === 3 && (<div className="space-y-6">
              <div>
                ...
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 hover:border-emerald-500 transition-colors">
                  <input type="file" id="plot-images" multiple accept="image/*" onChange={handleImageUpload} className="hidden" />
                  <label htmlFor="plot-images" className="flex flex-col items-center cursor-pointer">
                ...
                {imagePreviews.length > 0 && (<div className="grid grid-cols-3 gap-4 mt-4">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {documents.map((doc, index) => (<div key={doc.type} className="border border-slate-200 rounded-lg p-4 hover:border-emerald-500 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="font-medium text-slate-900 mb-1">
                          {getDocumentLabel(doc.type)}
                        </div>
                        <input type="file" id={`doc-${doc.type}`} accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file)
                            handleDocumentUpload(index, file);
                        }} className="hidden" />
                        <label htmlFor={`doc-${doc.type}`} className="inline-flex items-center space-x-2 text-sm text-emerald-600 hover:text-emerald-700 cursor-pointer">
                          <Upload className="w-4 h-4" />
                          <span>{doc.file ? 'Change file' : 'Upload document'}</span>
                        </label>
                      </div>
                      {doc.file && (<div className="flex items-center space-x-2 bg-emerald-50 px-3 py-1 rounded-lg">
              ...
              <div className="flex justify-between pt-4">
                <button type="button" onClick={() => setCurrentStep(2)} className="px-6 py-2 border-2 border-slate-300 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors">
                  Back
                </button>
                <button type="submit" className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg font-medium hover:from-emerald-700 hover:to-teal-700 transition-all">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {userPlots.length === 0 ? (<div className="p-12 text-center">
          ...
          <button onClick={() => setShowForm(true)} className="bg-emerald-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-emerald-700 transition-colors">
            Create Listing
          </button>
        </div>) : (<div className="divide-y divide-slate-200">
          {userPlots.map((plot) => (<div key={plot.id} className="p-6 hover:bg-slate-50 transition-colors">
            <div className="flex items-start space-x-4">
              <img src={plot.images[0]} alt={plot.title} className="w-24 h-24 object-cover rounded-lg" />
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    {plot.verification_status !== 'pending' && (<div className="flex items-center space-x-2">
                    ...
                    <button type="button" onClick={() => handleDelete(plot.id)} className="text-red-600 hover:text-red-700 text-sm font-medium">
                      Delete
                    </button>
                  </div>
                </div>
    ...
    {showPayment && pendingPlotId && (<ListingFeePayment plotId={pendingPlotId} onPaymentComplete={handlePaymentComplete} onCancel={() => {
      setShowPayment(false);
      setPendingPlotId(null);
    }} />)}
  </div>);
