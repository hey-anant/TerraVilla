  ...
  useEffect(() => {
    const fetchPlots = async () => {
      try {
        ...
        setAllPlots(fetchedPlots.filter(plot => plot.verification_status === 'verified'));
      } catch (error) {
        console.error("Error fetching plots in SearchPage:", error);
      } finally {
        setLoading(false);
      }
    };
...
<<<<<<< HEAD
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center">Loading properties...</div>;
  }
  ...
  return (<div className="min-h-screen bg-slate-50">
        ...
        <div className="bg-white rounded-xl shadow-xl p-4">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by location, city, or property name..." className="w-full pl-12 pr-4 py-3 text-slate-900 rounded-lg border-2 border-transparent focus:border-emerald-500 outline-none" />
            </div>
            <button onClick={() => setShowFilters(!showFilters)} className="bg-slate-100 text-slate-700 px-6 py-3 rounded-lg font-medium hover:bg-slate-200 transition-colors flex items-center justify-center space-x-2">
          ...
          {showFilters && (<div className="grid md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-slate-200">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">City</label>
              <select value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none text-slate-900">
            ...
            <div>
              ...
              <select value={selectedState} onChange={(e) => setSelectedState(e.target.value)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none text-slate-900">
            ...
            <div>
              ...
              <input type="number" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} placeholder="0" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none text-slate-900" />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Max Price (Lakhs)</label>
              <input type="number" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} placeholder="Unlimited" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none text-slate-900" />
            </div>
          </div>)}
        </div>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPlots.map((plot) => (<div key={plot.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl transition-all cursor-pointer group" onClick={() => setSelectedPlot(plot)}>
          <div className="relative">
            <img src={plot.images[0]} alt={plot.title} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
      ...
      {filteredPlots.length === 0 && (<div className="text-center py-16">
    ...
    {selectedPlot && (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-10">
          <h2 className="text-2xl font-bold text-slate-900">Property Details</h2>
          <button onClick={() => setSelectedPlot(null)} className="text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div className="grid md:grid-cols-2 gap-4 p-6">
          {selectedPlot.images.map((img, idx) => (<img key={idx} src={img} alt={`${selectedPlot.title} ${idx + 1}`} className="w-full h-64 object-cover rounded-xl" />))}
        </div>
        ...
        <div className="px-6 pb-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-slate-50 rounded-xl p-4">
              ...
              <div className="text-sm text-slate-600 text-center">Dimensions & Area</div>
              <div className="mt-2 space-y-1 text-center">
                <div className="text-sm font-semibold text-slate-900">{selectedPlot.length_ft} ft × {selectedPlot.width_ft} ft</div>
                <div className="text-xs text-slate-600">{selectedPlot.area_sqft.toLocaleString('en-IN')} sq ft</div>
              </div>
            </div>
          ...
          <div className="bg-slate-50 rounded-xl p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <a href={`tel:${user?.phone || selectedPlot.seller_phone}`} className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-3 rounded-lg font-medium hover:from-emerald-700 hover:to-teal-700 transition-all flex items-center space-x-2">
            ...
            <div className="px-6 pb-6">
              ...
              <div className="bg-slate-50 rounded-xl p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <a
                    href={`tel:${user?.phone || selectedPlot.seller_phone}`}
